...
FROM https://github.com/gonetz/GLideN64
 */
...
struct Atlas {
	...
	struct {
	 ...
	 Atlas(FT_Face face, int height) {
		...
		fprintf(stderr, "Initializing Atlas");
...
{
	char *fontName = "mona.ttf";

#ifdef OS_WINDOWS
	char * pSysPath = getenv("WINDIR");
	if (pSysPath == NULL)
		return false;
	sprintf(_strName, "%s/Fonts/%s", pSysPath, fontName);
#elif defined (ANDROID)
	sprintf(_strName, "/system/fonts/%s", fontName);
#elif defined (PANDORA)
	sprintf(_strName, "/usr/share/fonts/truetype/%s", fontName);
#else
    sprintf(_strName, "%s", fontName);
#endif
	return true;
}
...
void TextDrawer::init()
{
	...
	fprintf(stderr, "Init TextDrawer\n");
 ...
 */
void TextDrawer::renderText(const char *_pText, float _x, float _y, float sx, float sy) const
{
	...
	for (p = (const unsigned char *)_pText; *p; ++p) {
